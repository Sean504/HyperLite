/// Jan.ai backend — Default port: 1337, OpenAI-compat, GGUF
...
pub struct JanProvider { client: Client, base_url: String }
...
#[async_trait]
impl LocalProvider for JanProvider {
    fn id(&self)       -> &str { "jan" }
    fn name(&self)     -> &str { "Jan.ai" }
    fn kind(&self)     -> BackendKind { BackendKind::Jan }
    ...
    async fn list_models(&self) -> Result<Vec<LocalModel>> {
        openai_compat::list_models(&self.client, &self.base_url, BackendKind::Jan)
    ...
    async fn chat_stream(&self, messages: &[ChatMessage], model: &str, params: &GenerationParams) -> Result<mpsc::Receiver<StreamEvent>> {
        openai_compat::stream_chat(&self.client, &self.base_url, BackendKind::Jan, messages, model, params).await
    }
}
